...
async function main() {
  ...
  for (const category of categories) {
    console.log(`Kategori için talepler oluşturuluyor: ${category.name}`);
    
    // Create 2 listings for each category
    for (let i = 1; i <= 2; i++) {
      const subCategory = category.subcategories.length > 0 
        ? category.subcategories[i % category.subcategories.length] // Alternate subcategories if available
        : null;

      const title = `${category.name} - Örnek Talep ${i}`;
      const description = `Bu, ${category.name} kategorisinde oluşturulmuş ${i}. örnek taleptir. Detaylı bilgi için iletişime geçiniz.`;
      ...
      const listingData = {
        ...
        status: 'OPEN',
        imagesJson: JSON.stringify([]), // Empty images for now
        attributesJson: JSON.stringify(attributes),
        categoryId: category.id,
        subCategoryId: subCategory ? subCategory.id : undefined,
        ownerId: user.id,
        code: `${category.slug.substring(0, 3).toUpperCase()}-${Date.now()}-${i}` // Unique code
      };
